...
static int check_xyz(long x, long y, int z, struct xmlmapconfig * map) {
  int oob;
  long limit;
  ...
  if (oob)
    syslog(LOG_INFO, "got bad co-ords: x(%ld) y(%ld) z(%d)\n", x, y, z);
  
  return !oob;
}

#ifdef METATILE
mapnik::box2d<double> tile2prjbounds(struct projectionconfig * prj, long x, long y, int z) {

    long render_size_tx = MIN(METATILE, prj->aspect_x * (0x1L << z));
    long render_size_ty = MIN(METATILE, prj->aspect_y * (0x1L << z));

    double p0x = prj->bound_x0 + (prj->bound_x1 - prj->bound_x0)* ((double)x / (double)(prj->aspect_x * 0x1L<<z));
    double p0y = (prj->bound_y1 - (prj->bound_y1 - prj->bound_y0)* (((double)y + render_size_ty) / (double)(prj->aspect_y * 0x1L<<z)));
    double p1x = prj->bound_x0 + (prj->bound_x1 - prj->bound_x0)* (((double)x + render_size_tx) / (double)(prj->aspect_x * 0x1L<<z));
    double p1y = (prj->bound_y1 - (prj->bound_y1 - prj->bound_y0)* ((double)y / (double)(prj->aspect_y * 0x1L<<z)));

    syslog(LOG_DEBUG, "Rendering projected coordinates %i %ld %ld -> %f|%f %f|%f to a %ld x %ld tile\n", z, x, y, p0x, p0y, p1x, p1y, render_size_tx, render_size_ty);

    mapnik::box2d<double> bbox(p0x, p0y, p1x,p1y);
    return  bbox;
}

static enum protoCmd render(struct xmlmapconfig * map, long x, long y, int z, char *options, metaTile &tiles)
{
    unsigned int render_size_tx = MIN(METATILE, map->prj->aspect_x * (0x1L << z));
    unsigned int render_size_ty = MIN(METATILE, map->prj->aspect_y * (0x1L << z));
    ...
    try {
      ...
      syslog(LOG_ERR, "ERROR: failed to render TILE %s %d %ld-%ld %ld-%ld", map->xmlname, z, x, x+render_size_tx-1, y, y+render_size_ty-1);
...
#else //METATILE
static enum protoCmd render(Map &m, const char *tile_dir, char *xmlname, projection &prj, long x, long y, int z)
{
...
void *render_thread(void * arg)
{
    ...
    while (1) {
        ...
        if (item) {
            ...
            unsigned int size = MIN(METATILE, 0x1L << req->z);
                ...
                if (!strcmp(maps[i].xmlname, req->xmlname)) {
                    if (maps[i].ok) {
                        if (check_xyz(item->mx, item->my, req->z, &(maps[i]))) {
                            ...
                            if(sinfo.size > 0)
                                syslog(LOG_DEBUG, "DEBUG: START TILE %s %d %ld-%ld %ld-%ld, age %.2f days",
                                       req->xmlname, req->z, item->mx, item->mx+size-1, item->my, item->my+size-1,
                                       (tim.tv_sec-sinfo.mtime)/86400.0);
                            else
                                syslog(LOG_DEBUG, "DEBUG: START TILE %s %d %ld-%ld %ld-%ld, new metatile",
                                       req->xmlname, req->z, item->mx, item->mx+size-1, item->my, item->my+size-1);
                            ...
                            syslog(LOG_DEBUG, "DEBUG: DONE TILE %s %d %ld-%ld %ld-%ld in %.3lf seconds",
                        ...
                        cache_expire(maps[i].htcpsock,maps[i].host, maps[i].xmluri, req->x,req->y,req->z);
                            ...
                            syslog(LOG_WARNING, "Received request for map layer %s is outside of acceptable bounds z(%i), x(%ld), y(%ld)",
