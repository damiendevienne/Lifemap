...
int main(int argc, char **argv)
{
    ...
    if (all) {
        ...
        long lz = (0x1L << minZoom) - 1;

        if (minZoom == maxZoom) {
            if (maxX == -1) { maxX = lz; }
            if (maxY == -1) { maxY = lz; }
            if (minX > lz || minY > lz || maxX > lz || maxY > lz) {
                fprintf(stderr, "Invalid range, x and y values must be <= %ld (2^zoom-1)\n", lz);
                return 1;
            }
        }
    ...
    if (all) {
        ...
        for (z=minZoom; z <= maxZoom; z++) {
            long current_maxX = (maxX == -1) ? (0x1L << z)-1 : maxX;
            long current_maxY = (maxY == -1) ? (0x1L << z)-1 : maxY;
            printf("Rendering all tiles for zoom %d from (%d, %d) to (%ld, %ld)\n", z, minX, minY, current_maxX, current_maxY);
